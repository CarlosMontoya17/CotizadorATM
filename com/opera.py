...
def start():
    ...
    fPref.close() 
    fCurrent = open('currentTimesTwo.pbtxt', 'r') 
...
def browser(url):
    navPath = "C:/Users/"+str(username)+"/AppData/Local/Programs/Opera/launcher.exe"
    webbrowser.register('opera', None, webbrowser.BackgroundBrowser(navPath))
        ...
        try:
            if scores != [] and scores != None:
                ...
                if int(current) != replay:
                    currentFile = open("currentTimesTwo.pbtxt",'w')
                    current = int(current)
                    current+=1
                    currentFile.write(str(current))
                    currentFile.close()
                    os.startfile("sOpera.cmd")
